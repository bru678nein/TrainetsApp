...
| VIII — nada de auth propia | Proveedor en el ADR 0003, librería en el 0004, cierre de sesión en el 0005 — delegado, sin estado de sesión propio. La firma la verifica PyJWT; lo nuestro es decidir con los claims ya decodificados. Automático: `tests/test_tokens.py` firma con claves reales y prueba las falsificaciones. **Desde la feature 003 emitimos una credencial propia**: el token de invitación. No es una sesión ni sustituye al proveedor —quien acepta llega con su token de Clerk y el link sólo dice qué ficha reclama— pero es un secreto que emitimos y verificamos, y conviene que la fila lo diga en vez de que un revisor lo descubra. Se guarda hasheado, es de un solo uso, vence a los 7 días, y regenerar invalida el anterior; los cuatro verificados en `tests/test_aceptar_invitacion.py`. |
| IX — datos de desarrollo reales | `make seed` importa la planilla, y 4 de los 34 archivos de test la usan por la fixture `seeded`, que los saltea cuando falta. La proporción bajó a propósito: cada feature nueva arma su propio escenario para poder correr en CI, donde la planilla no existe. Los otros 9 construyen su propio escenario: los criterios 9 a 11 y los recorridos de rutas tienen que correr en CI, donde la planilla no existe. |
