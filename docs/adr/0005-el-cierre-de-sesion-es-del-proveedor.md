...
Al implementar el cierre de sesión aparecieron dos requisitos que se contradicen.

Uno pide que, cerrada la sesión, el token anterior deje de servir. El otro es una
regla del proyecto: **no se escribe manejo de sesiones a mano**, la autenticación
se delega. Cuando un requisito puntual choca contra una regla de esas, gana la
regla.
...
El requisito se enmienda para que diga lo que realmente pasa: cerrada la sesión
el proveedor deja de emitir tokens, y el token en curso deja de servir en cuanto
vence, dentro de esa ventana. Un requisito que promete inmediatez que el sistema
no da es peor que uno que declara su límite — quien lo lee cree que está
cubierto.
...
nuestro, que es exactamente lo que la regla saca de la mesa, y porque el
...
cambiar la regla y no por saltearla.

## Consecuencias

**A favor.** Cero estado de sesión en el backend: nada que expirar, nada que
limpiar, nada que se desincronice entre instancias. La regla se cumple
...
sigue sirviendo después del logout. Está documentada acá, que es la
...
de *otro* dispositivo desde el backend. Se pedía poder cerrar la sesión "desde
cualquier dispositivo", y con esta decisión eso significa que
