"""Claims to identity, or to a reason for rejection. Task T-004.

Written before the implementation, as article IV requires for the domain.

No network and no clock: `identificar` takes the decoded claims, what it expects
to find in them, and the current time. Reading the clock inside would make every
expiry test depend on when it runs, and a test that passes at 11:59 and fails at
12:00 gets deleted rather than fixed.

Signature verification is not here. It needs the JWKS, which needs the network,
which article I keeps out of the domain — that is the adapter, T-005. What lives
here is everything that can be decided by looking at the claims, which is where
the holes actually are: `azp` is the one people forget, and it is what stops a
token Clerk issued for another origin from working against this API.
"""

from __future__ import annotations

from datetime import UTC, datetime, timedelta

import pytest

from app.domain.identity import Identity, Motivo, TokenEsperado, identificar

AHORA = datetime(2026, 8, 8, 12, 0, tzinfo=UTC)

ESPERADO = TokenEsperado(
    emisor="https://clerk.example.com",
    origen_autorizado="https://app.example.com",
    algoritmos=frozenset({"RS256"}),
)


def claims(**cambios: object) -> dict[str, object]:
    """A valid token, with whatever the test needs broken."""
    base: dict[str, object] = {
        "sub": "user_abc",
        "iss": ESPERADO.emisor,
        "azp": ESPERADO.origen_autorizado,
        "exp": (AHORA + timedelta(minutes=5)).timestamp(),
        "nbf": (AHORA - timedelta(minutes=1)).timestamp(),
    }
    base.update(cambios)
    return base


class TestTokenValido:
    def test_devuelve_la_identidad(self):
        assert identificar(claims(), ESPERADO, "RS256", AHORA) == Identity("user_abc")

    def test_el_nbf_es_opcional(self):
        """Not every provider sends it. Its absence is not a reason to reject."""
        sin_nbf = claims()
        del sin_nbf["nbf"]
        assert identificar(sin_nbf, ESPERADO, "RS256", AHORA) == Identity("user_abc")


class TestVigencia:
    def test_vencido(self):
        vencido = claims(exp=(AHORA - timedelta(seconds=1)).timestamp())
        assert identificar(vencido, ESPERADO, "RS256", AHORA) is Motivo.VENCIDO

    def test_exp_exactamente_ahora_ya_vencio(self):
        """The boundary is decided here and not left to whoever reads it later."""
        justo = claims(exp=AHORA.timestamp())
        assert identificar(justo, ESPERADO, "RS256", AHORA) is Motivo.VENCIDO

    def test_nbf_en_el_futuro(self):
        futuro = claims(nbf=(AHORA + timedelta(minutes=1)).timestamp())
        assert identificar(futuro, ESPERADO, "RS256", AHORA) is Motivo.TODAVIA_NO_VALE

    def test_sin_exp_no_pasa(self):
        """A token with no expiry is a token that never expires."""
        sin_exp = claims()
        del sin_exp["exp"]
        assert identificar(sin_exp, ESPERADO, "RS256", AHORA) is Motivo.CLAIM_FALTANTE


class TestProcedencia:
    def test_otro_emisor(self):
        ajeno = claims(iss="https://otro-clerk.example.com")
        assert identificar(ajeno, ESPERADO, "RS256", AHORA) is Motivo.EMISOR_INCORRECTO

    def test_azp_de_otro_origen(self):
        """Criterion 7 of the spec, and the most common hole in this integration.

        Same issuer, valid signature, unexpired — and issued for another
        application. Without this check it works against this API.
        """
        otro = claims(azp="https://otra-app.example.com")
        assert identificar(otro, ESPERADO, "RS256", AHORA) is Motivo.ORIGEN_INCORRECTO

    def test_sin_azp_no_pasa(self):
        """Absence must not be a way around the check above."""
        sin_azp = claims()
        del sin_azp["azp"]
        assert identificar(sin_azp, ESPERADO, "RS256", AHORA) is Motivo.ORIGEN_INCORRECTO

    def test_algoritmo_inesperado(self):
        """`none` and HS256 against a public key are the classic forgeries."""
        assert identificar(claims(), ESPERADO, "none", AHORA) is Motivo.ALGORITMO_INESPERADO
        assert identificar(claims(), ESPERADO, "HS256", AHORA) is Motivo.ALGORITMO_INESPERADO


class TestSujeto:
    @pytest.mark.parametrize("sub", [None, "", "   "])
    def test_sin_sujeto_utilizable(self, sub):
        """`sub` becomes app_user.auth_user_id. Empty is not an identity."""
        roto = claims()
        if sub is None:
            del roto["sub"]
        else:
            roto["sub"] = sub
        assert identificar(roto, ESPERADO, "RS256", AHORA) is Motivo.CLAIM_FALTANTE


class TestOrdenDeLosChequeos:
    """Which reason wins when a token breaks more than one rule.

    It matters because criterion 6 makes `VENCIDO` a reason the client is told,
    so it knows renewing is worth a try, while everything else answers with a
    generic 401. Reporting `VENCIDO` for a token from another issuer would be
    telling a forger that their token is fine and just needs renewing.

    So provenance is checked first, and expiry last.
    """

    def test_vencido_y_de_otro_emisor_no_reporta_vencido(self):
        roto = claims(
            iss="https://otro-clerk.example.com",
            exp=(AHORA - timedelta(hours=1)).timestamp(),
        )
        assert identificar(roto, ESPERADO, "RS256", AHORA) is Motivo.EMISOR_INCORRECTO

    def test_vencido_y_de_otro_origen_no_reporta_vencido(self):
        roto = claims(
            azp="https://otra-app.example.com",
            exp=(AHORA - timedelta(hours=1)).timestamp(),
        )
        assert identificar(roto, ESPERADO, "RS256", AHORA) is Motivo.ORIGEN_INCORRECTO


def test_el_dominio_no_importa_infraestructura():
    """Article I, from inside and narrower than the CI grep.

    It reads the import statements, not the source text: the CI grep would flag
    a module that merely names a library in a comment, and a check that fires on
    prose gets worked around instead of obeyed. Here the only thing that counts
    is what the module actually pulls in.
    """
    import ast
    import inspect

    import app.domain.identity as modulo

    importados: set[str] = set()
    for nodo in ast.walk(ast.parse(inspect.getsource(modulo))):
        if isinstance(nodo, ast.Import):
            importados.update(a.name.split(".")[0] for a in nodo.names)
        elif isinstance(nodo, ast.ImportFrom) and nodo.module:
            importados.add(nodo.module.split(".")[0])

    prohibidos = {"sqlalchemy", "fastapi", "pydantic", "psycopg", "httpx", "jwt", "requests"}
    assert not (importados & prohibidos), f"el dominio importa {importados & prohibidos}"
